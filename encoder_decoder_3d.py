...
from models.resnet18_encoder import ResNetEncoder as resnet_encoder
...
#model
model = ResnetVAE(z_dim=10)
encoder_model = resnet_encoder()
encoder_model.load_state_dict(torch.load(os. getcwd()+'/encoder_model.pth'))
for i in encoder_model.parameters():
...
cl_strategy = VAETRAINING(
    model, optimizer, criterion=VAE_LOSS_resnet, train_epochs=5, mem_size=1000, device=device, evaluator=eval_plugin, train_mb_size=100, eval_mb_size=100)
